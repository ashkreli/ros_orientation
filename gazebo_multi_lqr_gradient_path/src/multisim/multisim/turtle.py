...
num_of_waypoints = 100 # arbitrary number
...
class Turtle(Node):
    ...
    def send_request(self):
        ...
        self.waypoints = []
        self.waypoints.append([calcs.pose_to_pt(self.req.initial_pose),[0.0,0.0,0.0]])
        
        # Calculate waypoint path
        self.calc_waypoint_path(False)
    ...
    def calc_waypoint_path(self,is_close):
        ...
        for i in range(0,num_of_waypoints):
            ...
            # instantiate the repulsive gradient
            rep_grad = Point(x=0.0,y=0.0,z=0.0)  
            
            # if they're close add repulsion
            if funct_is_close:
            ...
            next_waypoint = Point(x=self.waypoints[-1][0].x + step_size * -(tot_grad.x),
                                y=self.waypoints[-1][0].y + step_size * -(tot_grad.y),
                                z=self.waypoints[-1][0].z + step_size * -(tot_grad.z)
                                )
            # set the orientation of the next point
            (next_point_roll,next_point_pitch,next_point_yaw) = (0.0,self.waypoints[-1][1][1],self.waypoints[-1][1][2])
            if (calcs.dist(next_waypoint,self.waypoints[-1][0]) >= 10**(-10)):
    ...
    def approach_waypoint(self):
        ...
        if (calcs.dist(calcs.pose_to_pt(cur_pose), self.ref) > lin_marge):
            ...
            Q = numpy.array([[1.7, 0, 0],  # Penalize X position error 
                                        ...
                                        [0, 0, 0.8]]) # Penalize YAW ANGLE heading error  
            ...
            for turtle in self.all_names:
                # Add repulsive potential gradients from other turtles
                if (turtle != self.name):
                    if (calcs.dist(calcs.pose_to_pt((self.all_info[turtle])[0].pose),calcs.pose_to_pt(cur_pose)) <= LINEAR_MARGIN*2):
                        self.waypoints.clear()
                        self.waypoints.append([calcs.pose_to_pt(cur_pose),[0.0,0.0,0.0]])
                        self.calc_waypoint_path(True)
                self.get_logger().info(str(calcs.dist(calcs.pose_to_pt((self.all_info[turtle])[0].pose), self.ref)))

                if (calcs.dist(calcs.pose_to_pt((self.all_info[turtle])[0].pose), self.ref) <= lin_marge):
                    if (self.name == turtle):
                        break
                    self.get_logger().info("there is a robot within the radius from the ref")
                    lin_marge *= 2


            # get next waypoint
            dest = Point(x=self.waypoints[0][0].x,
                            y=self.waypoints[0][0].y,
                            z=self.waypoints[0][0].z
                        )
            ...
            self.get_logger().info(str(lin_marge))
            if calcs.dist(calcs.pose_to_pt(cur_pose), self.ref) > lin_marge:
