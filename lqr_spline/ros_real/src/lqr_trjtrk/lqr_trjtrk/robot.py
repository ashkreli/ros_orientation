...
class Robot(Node):
    ...
    def traj_path(self):
        ...
        '''waypts = [np.array([[0], [0]]), 
                  ...
                  np.array([[2], [-2]]), 
                  np.array([[3], [-3]])]
        max_vels = [MAX_LIN*0.5, MAX_LIN*0.5, MAX_LIN*0.5]
        ...
        waypts = [np.array([[0], [0]]), 
                  ...
                  np.array([[2], [0]]), 
                  np.array([[3], [-3]])]
        max_vels = [MAX_LIN*0.3, MAX_LIN*0.7, MAX_LIN*0.7]
        waypts = attach_t(waypts, max_vels)
        s_refs, u_refs = gen_s_u(waypts, N)
        dt = (waypts[-1][0] - waypts[0][0]) / N
        ...
        with open('ref_inputs.txt', 'w') as f3:
            for u_ref in self.u_refs:
                f3.write(np.array2string(u_ref) + '\n')
